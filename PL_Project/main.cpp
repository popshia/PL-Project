...
public:
    ...
    if ( walk ) {
      ...
      if ( walk->leftToken ) {
        if ( IsPrimitive( walk->leftToken ) ) {
          cout << "primitive: " << walk->leftToken->content << endl;
        } // if: if is primitive
        else {
          cout << "parameter: " << walk->leftToken->content << endl;
        } // else: is parameter
      } // if: has a left token
  ...
  */
  ...
  TokenStruct GetPrimitive() {
  
  } // CheckPrimitiveType()
  
  PrimitiveType CheckPrimitiveType( TokenStruct currentPrimitive ) {
